...
class OrientedHistogram:
    ...
    def calculate(
        self, vector_field: np.ndarray, mask: None | np.ndarray = None
    ) -> np.ndarray:
        vector_field = OrientedHistogram._cartesian_to_spherical(vector_field)
        hist = self._calculate_histogram(spherical_vector_field=vector_field, mask=mask)
    ...
    def _calculate_histogram(
        self, spherical_vector_field: np.ndarray, mask: None | np.ndarray = None
    ) -> np.ndarray:
        if mask is not None:
