...
from vroc.loss import TRELoss, mse_loss, ncc_loss, smooth_vector_field_loss
...
models = {
    f"level_{i_level}": models.DemonsVectorFieldBoosterForceLearning().to("cuda:0")
        ...
        for i_step, step in enumerate(registration_result):
            ...
            if ADDITIVE_MODE:
                vector_field_after_boost = (
                    step["vector_field_before"] + vector_field_boost
                )
            else:
                spatial_transformer = SpatialTransformer()
                vector_field_after_boost = spatial_transformer.compose_vector_fields(
                    vector_field_1=step["vector_field_before"],
                    vector_field_2=vector_field_boost,
                )

            vector_field_before = match_vector_field(
                vector_field=step["vector_field_before"], image_shape=full_image_shape
            )
            vector_field_after = match_vector_field(
                vector_field=step["vector_field_after"], image_shape=full_image_shape
            )
            vector_field_boost = match_vector_field(
                vector_field=vector_field_boost, image_shape=full_image_shape
            )
            ...
            smooth_after = smooth_vector_field_loss(
                vector_field=vector_field_after - vector_field_before, mask=fixed_mask
            )
            smooth_after_boost = smooth_vector_field_loss(
                vector_field=vector_field_boost, mask=fixed_mask
            )

            # > 0 (worse), < 0 (better)
            smoothness_ratio = smooth_after_boost / smooth_after - 1
            # just penalize worsening of smoothness,
            smoothness_ratio = torch.clip(smoothness_ratio, min=0.0)

            tre_difference = tre_after - tre_before
            tre_difference_boost = tre_after_boost - tre_before
            tre_improvement = tre_difference_boost - tre_difference

            tre_ratio = tre_improvement / (1 / params["iterations"] * tre_before)

            logger.info(
                f"[{i_epoch}/{i_step}] Case {case}, {tre_before=:.2f}, {tre_after=:.2f}, {tre_after_boost=:.2f}, {tre_after_boost-tre_after=:.2f}, {smoothness_ratio=:.2f}, {tre_ratio=:.2f}"
            )

            loss = tre_improvement + smoothness_ratio

            logger.info(f"[{i_epoch}/{i_step}] Case {case}, {loss.item()=:.2f}")
            loss.backward()
            optimizer.step()
