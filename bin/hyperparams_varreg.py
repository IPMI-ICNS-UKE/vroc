...
for case in dirlab_cases:
    ...
    fixed_LM = read_landmarks(os.path.join(data_path, 'extremePhases', f'Case{case}_300_T00_xyz.txt'))
    fixed_imgs_LM.append(transform_landmarks_and_flip_z(fixed_LM, orig_ref))
    moving_LM = read_landmarks(os.path.join(data_path, 'extremePhases', f'Case{case}_300_T50_xyz.txt'))
...
def reg(params):
    ...
    for i, _ in enumerate(dirlab_cases):
        model = TrainableVarRegBlock(patch_shape=sitk.GetArrayFromImage(ref_imgs[i]).shape,
        ...
        _, _, _, vf, metrics = model.forward(fixed_imgs[i], mask_imgs[i], moving_imgs[i], ref_imgs[i].GetSpacing())
        ...
        vector_field_scaled = sitk.Compose(
            [sitk.VectorIndexSelectionCast(vector_field, i) * sp for i, sp in enumerate(ref_imgs[i].GetSpacing())])
        vector_field_scaled.CopyInformation(ref_imgs[i])
        vf_transformed = sitk.DisplacementFieldTransform(vector_field_scaled)
        TRE_world = target_registration_errors_snapped(vf_transformed, fixed_imgs_LM[i], moving_imgs_LM[i], ref_imgs[i],
                                                       world=True)
